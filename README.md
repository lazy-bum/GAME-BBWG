# 奔奔王国批量兑换

一个用于管理《奔奔王国》账号、批量兑换礼包码，并自动抓取新兑换码后触发兑换的 Web 工具。

## 技术栈

- Node.js 22+
- TypeScript
- Express
- sqlite3

## 主要功能

- 批量新增账号，按行导入并自动去重
- 支持账号与分组的备份导出、备份导入恢复
- 账号列表管理，支持搜索、删除、清空、拉黑状态和拖拽排序
- 管理员可维护账号分组，并批量把账号分配到指定分组
- 批量兑换，支持按全部账号、指定分组或指定用户兑换，并支持失败重试和进度实时推送
- 批量兑换和自动兑换会按账号分组优先级执行
- 自动保存和刷新兑换站点 `REDEEM_TOKEN`
- 自动抓取兑换码并写入 `redeem_codes`
- 新兑换码自动给当前所有账号兑换
- 新增账号后自动兑换最新一个兑换码，批量新增时会按队列执行
- 支持管理员账号和临时账号两种登录角色
- 支持访问日志和 IP 黑名单

## 账号分组与兑换优先级

账号分组仅管理员可见和维护。管理员可以在首页进入“分组管理”，新增分组并设置：

- 分组名称
- 优先级：数值越大，兑换越靠前
- 排序：数值越小，同优先级下越靠前

账号列表页支持按分组切换 tab，默认展示“未分组”账号；也可以切到具体分组或“全部”。管理员可以勾选当前列表中的账号，批量分配到某个分组，也可以选择“未分组”清除账号分组。

兑换顺序统一按以下规则计算：

1. 已分组账号优先于未分组账号。
2. 分组优先级 `priority` 越大越先兑换。
3. 分组排序 `sort_order` 越小越先兑换。
4. 同一分组内按账号列表拖拽排序 `sort_order` 执行。
5. 排序值相同时，创建时间更早的账号更靠前。

这套排序同时作用于手动批量兑换、按分组定向兑换、自动兑换、失败账号重试和新增账号后的自动兑换。

## 账号导入导出

账号列表页提供两类导入能力：

- “新增账号”页面：按行填写账号 ID，系统会在线拉取资料并新增不存在的账号。
- “用户列表”页面：管理员可直接导出当前账号备份，也可导入此前导出的备份文件。

备份导出说明：

- 导出文件为 JSON 格式。
- 包含账号、分组、黑名单状态、排序、账号详情等数据。
- 适合迁移环境、做手动备份、恢复误删前的数据状态。

备份导入说明：

- 仅管理员可操作。
- 导入时不会先清空现有数据，而是按 `accountId` 和 `groupId` 合并更新。
- 已存在的账号和分组会更新为备份中的内容，不存在的会新增。
- 如果备份中的账号引用了不存在的分组，该账号会自动回退为未分组。

## 兑换码来源

项目支持两种兑换码来源，但同一进程只会启用其中一种。

### 默认模式：TapTap

不带额外参数启动时，默认抓取 TapTap《奔奔王国》官方公告：

```text
https://www.taptap.cn/app/759692/topic?type=official
```

服务端会读取 TapTap 官方帖子列表接口，解析帖子标题和正文中的兑换码。

### 微信模式：微信公众号

使用微信模式启动时，不会再启动 TapTap 抓取。

微信模式会登录微信公众平台后台，定时检查目标公众号文章列表。默认目标公众号 `fakeid`：

```text
MzE5MTIzOTAzNQ==
```

文章列表中已经包含文章详情链接，服务端会使用类似下面的微信移动端 User-Agent 请求详情页：

```text
https://mp.weixin.qq.com/s/xxxxxx
```

然后从文章正文中解析兑换码。

微信模式会优先复用上一次登录保存的 `token/cookie/userAgent`。启动时会先校验 `config.json` 中的微信登录态；校验通过就直接启动微信轮询，不再扫码。校验失败或没有历史登录态时，终端才会输出二维码；扫码并在手机上确认后，会把新的登录态保存到 `config.json`。终端二维码会使用兼容宝塔项目运行日志的纯文本渲染；如果日志二维码仍无法识别，也会保存本地二维码图片到 `data/wechat-login-qrcode.jpg`。

## 抓取规则

- 服务启动后会立即抓取一次。
- 服务运行期间每 1 分钟抓取一次。
- 抓到的兑换码会写入 `redeem_codes` 表。
- 同一个兑换码按 `code` 去重。
- 重复抓到同一个兑换码时，只更新来源标题、正文、发布时间和最后发现时间。
- 只解析标题或正文中类似 `兑换码：xxx`、`礼包码：xxx`、`CDK：xxx` 的内容。
- TapTap 兑换码发布时间来自帖子发布时间。
- 微信兑换码发布时间来自微信公众号文章发布时间。
- 微信模式会把文章记录写入 `wechat_articles` 表。
- 微信登录态失效时，会停止微信抓取并输出日志，但不会影响 Web 服务继续运行。

## 自动兑换规则

- 只有新入库的兑换码会触发自动兑换。
- 只自动兑换最近 24 小时发布的兑换码。
- 超过 24 小时的历史兑换码只入库，不自动兑换。
- 如果一次抓到多个新兑换码，会按队列顺序逐个兑换，不会并发执行。
- 自动兑换期间会暂停当前启用的兑换码来源轮询，兑换结束后恢复。
- 自动兑换前会检查 `REDEEM_TOKEN`；如果为空或需要刷新，会自动从兑换站点抓取并保存。
- 每个兑换码会在 `redeem_code_redemptions` 表记录兑换状态，避免重复触发。
- 自动兑换会对当前所有未删除、未拉黑账号执行，并按账号分组优先级排序。
- 自动兑换结束后，如果存在失败账号，会只针对失败账号额外重试 1 次，避免死循环。
- 自动兑换结果会记录总数、成功数、已领取数、失败数和错误信息。

## 启动方式

安装依赖：

```bash
npm install
```

开发模式：

```bash
npm run dev
```

生产模式：

```bash
npm run build
npm run start
```

生成发布包（不上传源码）：

```bash
npm run package:release
```

生成后会得到：

```text
release/bbwg-release.tar.gz
```

默认只包含生产运行所需文件：

- `dist/`
- `public/`
- `package.json`
- `package-lock.json`
- `.env.example`
- `.well-known/`（如果目录存在）

如果你要把当前运行数据一起迁移，也可以手动执行：

```bash
node scripts/package-release.mjs --with-data --with-config
```

这会额外把以下运行时文件打进包里：

- `data/`（sqlite 数据库、二维码图片等）
- `config.json`（自动保存的兑换 token、微信登录态等）

服务器解压后执行：

```bash
npm ci --omit=dev
cp .env.example .env
node dist/server.js
```

默认地址：

```text
http://localhost:3458
```

## 微信模式启动

生产模式推荐使用：

```bash
npm run build
npm run start:wechat
```

如果需要忽略已有登录态并强制重新扫码登录，可以使用：

```bash
npm run build
npm run start:wechat:login
```

如果只想刷新微信登录态，扫码成功后保存到 `config.json` 并退出，可以使用：

```bash
npm run wechat:login
```

开发模式可以使用：

```bash
npm run dev -- --wechat
```

说明：

- `npm run start:wechat` 等价于 `node dist/server.js --wechat`。
- `npm run start:wechat:login` 等价于 `node dist/server.js --wechat --force-wechat-login`。
- `npm run wechat:login` 只执行微信扫码登录并更新 `config.json`，不会启动 Web 服务。
- 微信模式启动后会先校验 `config.json` 中已有的公众号后台登录态。
- 已有登录态有效时会跳过扫码，直接启动微信文章轮询。
- 使用 `--force-wechat-login` 时会跳过登录态校验，直接进入扫码登录流程。
- 没有历史登录态或登录态失效时，才会进入扫码登录流程。
- 扫码成功后会保存新的 `token/cookie/userAgent`，供下次启动校验复用。
- 如果扫码失败、超时或登录态不可用，Web 服务仍会启动，只是不启用微信轮询。
- 如果运行中微信登录态过期，微信轮询会停止，Web 服务和手动兑换功能不受影响。

## 环境变量

复制 `.env.example` 到 `.env` 后配置：

- `PORT`：服务端口，默认 `3458`
- `REDEEM_TOKEN`：兑换站点签名 TOKEN，可以为空，自动兑换前会尝试自动获取
- `SESSION_SECRET`：登录会话签名密钥
- `WECHAT_MP_FAKEID`：微信模式目标公众号 fakeid，默认 `MzE5MTIzOTAzNQ==`

## 常见日志说明

```text
auto redeem skipped for old code xxx, publishedAt=...
```

表示该兑换码已经入库，但发布时间超过 24 小时，所以不会自动兑换。

```text
WeChat login failed, web server will continue without WeChat polling
```

表示微信扫码登录失败或超时，服务会继续启动，但不会执行微信公众号抓取。
